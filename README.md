...
**The "good enough?" check explains itself.** Instead of a plain yes/no, it always records *why* — "low similarity," "borderline, but passed a closer read," etc. That reasoning is visible in every response, so you can see why an answer was trusted or refused, not just that it was.
